    ...
    def generate(self, prompt: str, n: int):
        ...
        while True:
            try:
                ...
                return [
                    Image.open(io.BytesIO(base64.b64decode(d.b64_json)))
                    for d in resp.data
                ]
                ...
                label = error_label(e)
                log(
                    f"[{hhmm()}] {label} → wait {wait_s}s then retry (attempt {attempts_generic}/{self.max_retries})"
                )
                time.sleep(wait_s)
        ...
        while True:
            try:
                ...
                if isinstance(e, openai.BadRequestError) and is_moderation_block(e):
                    if moderation_retry_used:
                        log(
                            f"[{image_path.name}] Bad Request (moderation) again → skipping"
                        )
                        return None
                    log(
                        f"[{image_path.name}] Bad Request (moderation) → retrying once…"
                    )
                    moderation_retry_used = True
                    continue
                ...
                label = error_label(e)
                log(
                    f"[{image_path.name}] {label} → wait {wait_s}s then retry (attempt {attempts_generic}/{self.max_retries})"
                )
                time.sleep(wait_s)
...
class ImageGenerator:
    ...
    def generate_images(
        self, prompt: str, num_images: int, output_dir: Path | None, batch_size: int = 2
    ):
        if self.model == "gpti1":
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=50,
        help="Batch size for SD/FLUX to control VRAM",
    )
    ...
    if args.edit_image_path:
        ...
        gen.generate_images(
            args.prompt, args.num_images, args.output_dir, batch_size=args.batch_size
        )
