...
from src.plotting.dataframe_from_prettytables_loader import load_prettytables_as_dataframe
...
def load_dataframe_from_experiment_set(set_prefix: str) -> pd.DataFrame:
    ...
    result = {
        ...
        for folder in root_dir.iterdir() if folder.is_dir() and folder.name.startswith(set_prefix)
    }
...
def load_metrics_from_experiment_set(set_prefix: str) -> pd.DataFrame:
    ...
    concats.columns = ["example_file", "Model", "Layer", "Concept", "Similarity", "Folder"]
...
if __name__ == "__main__":
    ...
    mask_false_attributions = attributions["Concept"].isin(similarities["Concept"].unique())
    ...
    generated_attributions_on_real_concept = generated_attributions.drop_duplicates(subset=['Model', 'Concept', 'Layer', 'Folder'], keep="first")
    ...
    index1 = generated_attributions_on_real_concept.set_index(['Model', 'Concept', 'Layer', 'Folder'])
    index2 = similarities.set_index(['Model', 'Concept', 'Layer', 'Folder'])
    joined = index1.join(index2, on=['Model', 'Concept', 'Layer', 'Folder']).reset_index()

    ordered_cols = ['Model', 'Layer', 'Concept', 'Examples'] + [col for col in joined.columns if col not in ['Model', 'Layer', 'Concept', 'Examples']]
    joined = joined[ordered_cols].drop(columns=["Folder"])
    last_layers = joined.groupby(['Model', 'Concept', 'Examples'])['Layer'].max().unique()
    plotting_frame = joined[joined["Layer"].isin(last_layers)]
    plotting_frame = plotting_frame.copy()
    plotting_frame[['mimick', 'LLM']] = plotting_frame['Concept'].str.split('_', n=1, expand=True)

    true_attributions = true_attributions.copy()
    true_attributions['mimick'] = true_attributions['Concept']
    true_attributions['LLM'] = "Original"
    true_attributions = true_attributions[true_attributions["Layer"].isin(last_layers)].drop_duplicates(["Model", "Class", "Concept"])
    ...
    for cls, class_group in plotting_frame.groupby('Class'):
        ...
        for idx, (model, mimick) in enumerate(model_mimick_combos.values):
            ...
            group_df = class_group[(class_group['Model'] == model) & (class_group['mimick'] == mimick)]
